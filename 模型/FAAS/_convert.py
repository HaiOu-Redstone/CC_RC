...
def convert(src_name, out_name):
    with open(os.path.join(SRC, src_name), "r", encoding="utf-8-sig") as f:
        data = json.load(f)
    elements = []
...
for src, out in NA.items():
    convert(src + ".json", out)
